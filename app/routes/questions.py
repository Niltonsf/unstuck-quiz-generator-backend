...
from app.models import ValidateAnswerRequest, AppException, CreateQuizRequest, DecryptQuizRequest
...
async def upload_pdf(file: UploadFile = File(...)):
    try:        
        if file.content_type != "application/pdf":
            raise AppException(400, "INVALID_FILE_TYPE", "Invalid file type. Only PDF allowed.")
        ...
        if num_pages > 1:            
            raise HTTPException(status_code=400, detail="PDF has more than 1 page.")
...
async def create_quiz(data: CreateQuizRequest):
    try:        
        encryptedAnswers = encrypt_question_answer(data.data)
...
@router.post("/decrypt-quiz")
async def decrypt_quiz(request: DecryptQuizRequest):
    try: 
        decrypted_questions = []

        for question in request.data:
            decrypted_answer = [decrypt_answer(ans) for ans in question.answer]
            updated_question = question.copy(update={"answer": decrypted_answer})
            decrypted_questions.append(updated_question)
